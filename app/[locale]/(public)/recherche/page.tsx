...
export default function PageRecherche() {
  ...
  const [typeSelectionne, setTypeSelectionne] = useState<TypeLogement | null>(typeUrl ?? null);
  ...
  const resultats = logementsDemo.filter((logement) => {
    ...
    const correspondPrixMin = prixMin ? logement.prix_mensuel >= Number(prixMin) : true;
    const correspondPrixMax = prixMax ? logement.prix_mensuel <= Number(prixMax) : true;
    return correspondVille && correspondType && correspondPrixMin && correspondPrixMax;
  });
